...
import { FaPlusCircle, FaSpinner, FaArrowDown, FaRandom } from "react-icons/fa";
...
const FeedPage = () => {
  ...
  const toggleSortMethod = () => {
    ...
    toast.info(
      `${newSort === "latest" ? "최신순" : "랜덤"} 정렬로 변경되었습니다.`
    );
  };
  ...
  const toggleFilterOption = () => {
    ...
    toast.info(
      `${newFilter === "all" ? "전체 피드" : "팔로잉 피드"}로 전환되었습니다.`
    );
  };
        ...
        {isPulling && (
          ...
          >
            <div className="flex flex-col items-center">
              <FaArrowDown
                className={`text-2xl ${
                  pullY > 50 ? "text-black" : "text-gray-400"
                }`}
                style={{
                  transform: `rotate(${Math.min(pullY * 3.6, 180)}deg)`,
                }}
              />
