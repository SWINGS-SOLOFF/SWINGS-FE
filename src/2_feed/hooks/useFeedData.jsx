...
export const useFeedData = (userId, currentUser) => {
  ...
  const refreshFeeds = async () => {
    ...
    try {
      ...
      const processedFeeds = fetchedFeeds.map((feed) => ({
        ...
        commentCount: feed.commentCount ?? feed.comments?.length ?? 0,
        comments: feed.comments ?? [],
        username:
          feed.username ?? feed.user?.username ?? feed.user?.name ?? "익명",
        userProfilePic: feed.user?.profilePic || feed.user?.userImg || null,
      }));
  ...
  const handleCommentInputChange = (feedId, commentText) => {
    setFeeds(
      feeds.map((feed) =>
        feed.feedId === feedId ? { ...feed, newComment: commentText } : feed
      )
    );
  };
  ...
  const handleCommentSubmit = async (feedId, commentText) => {
    ...
    try {
      // API 호출로 댓글 추가
      const newComment = await socialApi.addComment(
        feedId,
        currentUser.userId,
        commentText
      );
